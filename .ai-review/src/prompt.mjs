...
// Hard examples — the model MUST classify these as critical if it sees them.
...
export function buildSystemPrompt(rulesCorpus, focusAreas) {
...
export function buildUserPrompt({ prTitle, prBody, prAuthor, filesDiff }) {
  const header = [
    `## PR metadata`,
    `- Title: ${prTitle || '(none)'}`,
