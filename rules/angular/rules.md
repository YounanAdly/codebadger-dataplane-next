...
Platform detection signals: `angular.json`, `src/app/**`, Angular workspace config.

This file is the **loading index** for Angular review rules. The rule loader reads the
`## Instruction files` section below and inlines every referenced instruction file whose
`applyTo` scope matches the pull request's changed files — so a template-only PR does not
load API or Formly rules. The project rulebook below always loads.

## Instruction files

Independent, self-contained instruction packages under `.github/instructions/angular/`:

- .github/instructions/angular/accessibility.instructions.md — WCAG AA with @angular/cdk primitives, focus management, live regions
- .github/instructions/angular/api-calls.instructions.md — Angular 22 async contract: httpResource stack, cancellation, status matrix, resilience
- .github/instructions/angular/auth.instructions.md — the shipped dual-token cookie auth flow, interceptors, guards
- .github/instructions/angular/error-handling.instructions.md — centralized error handling, translated toasts, no swallowed failures
- .github/instructions/angular/formly-forms.instructions.md — JSON-driven Formly forms, registered custom types, validators
- .github/instructions/angular/i18n.instructions.md — ngx-translate keys, en/ar parity, RTL
- .github/instructions/angular/shared-reuse.instructions.md — reuse-first inventory check, standalone component conventions
- .github/instructions/angular/styling-themes.instructions.md — design tokens, theme variables, RTL-safe layout, motion
